...
| 场景 | 原命令 | 优势 |
|------|---------|------|
| **TCP 数据接收** | `0x2C` | 批量读取 (1KB/次) 替代单字节，支持 TCP/UDP/TLS |
| **WiFi 扫描** | `0x27` | 后台扫描，流式返回每个 AP 完整信息 |
| **文件读取** | `0x61` | 自动批量读取，无需手动分块 |

**WiFi 扫描流数据格式 (每个 AP):**
```
[SSID长度(1)][SSID(N)][RSSI(4)][加密类型(1)][BSSID(6)][信道(4)]
```
